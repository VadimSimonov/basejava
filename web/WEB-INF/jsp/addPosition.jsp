...
<section>
    <c:set var="org" value='<%=request.getParameter("org")%>'></c:set>

    <form method="post" action="resume?action=addPosition&org=${org}" enctype="application/x-www-form-urlencoded">
            ...
            <c:choose>
                ...
                <c:when test="${type.name()=='EXPERIENCE' || type.name()=='EDUCATION' }">
                    <c:forEach items="<%=((OrganizationSection) section).getOrganizations()%>" var="organizations">
                        ...
                        <input type="hidden" name="${type}_url_${organizations.homePage.name}" value="${organizations.homePage.url}">
                        <c:forEach items="${organizations.positions}" var="positions">
                           <input type="hidden" name="${type}_title_${organizations.homePage.name}" value="${positions.title}">
                            ...
                            <input type="hidden" name="${type}_startDate_${organizations.homePage.name}" value="${positions.startDate}">

                                <fmt:parseDate value="${positions.endDate}" pattern="yyyy-MM-dd" var="parsedDate" type="date" />
                                <fmt:formatDate value="${parsedDate}" var="stdDatum" type="date" pattern="yyyy/MM" />
                                <input type="hidden" name="${type}_endDate_${organizations.homePage.name}" value="${positions.endDate}">

                            <input type="hidden" name="${type}_description_${organizations.homePage.name}" value="${positions.description}">
                        </c:forEach>
                    </c:forEach>
                </c:when>
            </c:choose>
        ...
        <br/><br/><b>Должность:</b>
        <dd><input name="${type}_title_${org}" type="text" size="50" value=""><br/></dd>
        <br/><b>Дата начала:</b>
        <dd><input name="${type}_startDate_${org}" type="date" size="50" value=""></dd><br/>
        <b>Дата окончания:</b>
        <dd><input name="${type}_endDate_${org}" type="date" size="50" value=""></dd>
        <br/><b>Описание:</b><br/>
        <textarea name="${type}_description_${org}" cols="70"></textarea>
